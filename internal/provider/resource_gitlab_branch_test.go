...
func TestAccGitlabBranch_basic(t *testing.T) {
    // Set up project to use in the test.
    ...
    resource.Test(t, resource.TestCase{
        ...
        Steps: []resource.TestStep{
            // Create the branch
            {
                Config: testAccGitlabBranchConfig(project.ID, rInt, project.DefaultBranch),
                Check: resource.ComposeTestCheckFunc(
                    ...
                    testAccCheckGitlabBranchAttributes(&b, &testAccGitlabBranchExpectedAttributes{
                        Name: fmt.Sprintf("example-%d", rInt),
                    }),
                ),
            },
            // Test ImportState
            ...
            {
                Config: testAccGitlabBranchConfig(project.ID, rModifyInt, project.DefaultBranch),
                Check: resource.ComposeTestCheckFunc(
                    testAccCheckGitlabBranchExists("gitlab_branch.this", &b),
                    testAccCheckGitlabBranchAttributes(&b, &testAccGitlabBranchExpectedAttributes{
                        Name: fmt.Sprintf("example-%d", rModifyInt),
                    }),
                ),
            },
        },
...
func testAccGitlabBranchConfig(projectID int, rInt int, ref string) string {
    return fmt.Sprintf(`
