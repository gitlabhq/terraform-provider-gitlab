...
- `require_password_to_approve` (Boolean, Deprecated) Set to `true` to require authentication to approve merge requests. Deprecated, to be removed in 20.0. Use `require_reauthentication_to_approve` instead.
- `require_reauthentication_to_approve` (Boolean) If true, requires approver to reauthenticate before adding the approval.
